...
struct seg_cmp {
    bool operator()(const pkt_buf* a, const pkt_buf* b) const {
        return *a < *b;
    }
};

struct TCB {
    pkt_buf* rx_ready_head;
    ...
    void process(std::vector<int>& rx_free_stk) {
        while (!rx_out_of_order.empty()) {
            pkt_buf* rx_seg = rx_out_of_order.peek_front();
            tcp_hdr* tcp = get_tcp_hdr(rx_seg);
    ...
    void handle_ack(u32 ack_num, std::vector<int>& tx_free_stk) {
        ...
        while (pkt_buf* seg = tx_unacked.peek_front()) {
            u32 seq_end = seg->meta.seq + seg->meta.payload.size() + !!(get_tcp_hdr(seg)->control & FIN_FLAG);
...
private:
    void append_ready(pkt_buf* rx_seg) {
