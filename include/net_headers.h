...
constexpr int TCP_MAX_PAYLOAD_SZ = BUF_SZ - TCP_TOTAL_METADATA_SZ;
...
inline eth_hdr* get_eth_hdr(pkt_buf* pb) {
    return reinterpret_cast<eth_hdr*>(pb->dma_buf);
}
inline ip_hdr* get_ip_hdr(pkt_buf* pb) {
    return reinterpret_cast<ip_hdr*>(pb->dma_buf + ETH_HDR_SZ);
}
inline tcp_hdr* get_tcp_hdr(pkt_buf* pb) {
    return reinterpret_cast<tcp_hdr*>(pb->dma_buf + ETH_HDR_SZ + IP_HDR_SZ);
}

inline std::span<std::byte> get_tcp_payload(pkt_buf* pb) {
...
inline std::span<std::byte> get_tcp_options(pkt_buf* pb) {
    auto* tcp = get_tcp_hdr(pb);
